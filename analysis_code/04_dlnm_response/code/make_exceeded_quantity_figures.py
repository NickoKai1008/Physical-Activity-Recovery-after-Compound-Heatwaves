"""Render exactly two complete EQ Fig. 19a+b figures from frozen plotting nodes."""
    ...
    mpl.rcParams.update(
        {
            ...
            "svg.fonttype": "none",
            "svg.hashsalt": "heatpa-fig19-eq-twofigure-20260809",
            "pdf.fonttype": 42,
        }
    )
...
def validate_inputs(
    ...
    if version == "p99":
        if not metadata["basis_specification"].eq(
            "shared p50/p90 natural spline within common support"
        ).all():
            raise RuntimeError("p99: EQ basis is not the locked shared p50/p90 specification")
        if not (
            metadata["knot_1"].gt(0)
            & metadata["knot_2"].gt(metadata["knot_1"])
            & metadata["knot_2"].le(metadata["common_support_high"])
        ).all():
            raise RuntimeError("p99: invalid two-knot EQ basis metadata")
    else:
        if not metadata["basis_specification"].eq(
            "shared p25 natural spline within common support"
        ).all():
            raise RuntimeError("p80: EQ basis is not the locked shared p25 specification")
        if not (
            metadata["knot_1"].gt(0)
            & metadata["knot_1"].le(metadata["common_support_high"])
            & metadata["knot_2"].isna()
        ).all():
            raise RuntimeError("p80: invalid one-knot EQ basis metadata")
...
def plot_histogram(
    ...
    if version == "p99":
        ...
        ax.set_xlabel("Excess heat (EQ)", fontsize=8.8, labelpad=1.5)
...
def render(
    ...
    *,
    ...
    for row, cluster in enumerate(range(1, 5)):
        ...
        af_padding = 0.14 if version == "p99" else 0.10
        ax_af.set_ylim(*padded_limits(y_values, fraction=af_padding))
...
def final_checks() -> None:
    ...
    expected_outputs = {
        "Fig19a_EQ_p99_curve_plot_data.csv",
        "Fig19b_EQ_p99_AF_plot_data.csv",
        "Extended_Fig19a_EQ_p80_curve_plot_data.csv",
        "Extended_Fig19b_EQ_p80_support_AF_plot_data.csv",
    }
    expected_figures = {
        "Fig19_EQ_p99.png",
        "Fig19_EQ_p99.svg",
        "Extended_Fig19_EQ_p80_support_coded_AF.png",
        "Extended_Fig19_EQ_p80_support_coded_AF.svg",
    }
    ...
    p99 = pd.read_csv(OUTPUT / "Fig19a_EQ_p99_curve_plot_data.csv")
    ...
    p99_af = pd.read_csv(OUTPUT / "Fig19b_EQ_p99_AF_plot_data.csv")
    ...
    for _, af_row in p99_af_source.iterrows():
        curve_panel = panel_row(
            p99_source, int(af_row["cluster"]), str(af_row["heatwave_type"])
        ).sort_values("exposure")
        exposure = float(af_row["exposure_value"])
        curve_eta = float(
            np.interp(exposure, curve_panel["exposure"], curve_panel["log_rr"])
        )
        expected_af = 100.0 * (1.0 - np.exp(-curve_eta))
        if not np.isclose(
            expected_af, float(af_row["af_percent"]), rtol=0.0, atol=1e-8
        ):
            raise RuntimeError("p99 curve estimate is inconsistent with panel b")
    p80 = pd.read_csv(OUTPUT / "Extended_Fig19a_EQ_p80_curve_plot_data.csv")
    ...
    for cluster in range(1, 5):
        for heatwave_type in TYPE_ORDER:
            one_knot = panel_row(p80_source, cluster, heatwave_type).sort_values("exposure")
            two_knot = panel_row(p99_source, cluster, heatwave_type).sort_values("exposure")
            comparison_x = one_knot["exposure"].to_numpy(float)
            two_knot_at_p80 = np.interp(
                comparison_x,
                two_knot["exposure"].to_numpy(float),
                two_knot["log_rr"].to_numpy(float),
            )
            specification_differences.append(
                float(np.max(np.abs(one_knot["log_rr"].to_numpy(float) - two_knot_at_p80)))
            )
    if max(specification_differences) <= 1e-4:
        raise RuntimeError("p80 one-knot and p99 two-knot curve objects are indistinguishable")
    p80_af = pd.read_csv(OUTPUT / "Extended_Fig19b_EQ_p80_support_AF_plot_data.csv")
    ...
    cross_window_af = p80_af_source.merge(
        p99_af_source,
        on=["cluster", "heatwave_type", "exposure_percentile"],
        suffixes=("_p80", "_p99"),
        validate="one_to_one",
    ...
    if np.allclose(
        cross_window_af["af_percent_p80"],
        cross_window_af["af_percent_p99"],
        rtol=0.0,
        atol=1e-4,
    ):
        raise RuntimeError("p80 one-knot and p99 two-knot AF objects are indistinguishable")
...
def main() -> None:
    ...
    render(
        ...
        p99_af,
        version="p99",
        output_stem="Fig19_EQ_p99",
        output_csv="Fig19a_EQ_p99_curve_plot_data.csv",
        af_output_csv="Fig19b_EQ_p99_AF_plot_data.csv",
    )
    ...
    render(
        ...
        p80_af,
        version="p80",
        output_stem="Extended_Fig19_EQ_p80_support_coded_AF",
        output_csv="Extended_Fig19a_EQ_p80_curve_plot_data.csv",
        af_output_csv="Extended_Fig19b_EQ_p80_support_AF_plot_data.csv",
    )
    final_checks()
